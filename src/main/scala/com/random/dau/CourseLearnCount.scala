...
import com.random.bean.LearnModel
...
object CourseLearnCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stream: InputDStream[ConsumerRecord[String, String]] = ConsumerUtil.getCourseLearnLog("course_learn", ssc, offsetMap)

    //解析json数据
    val dsStream = stream.mapPartitions(partitions => {
      partitions.map(item => {
        val json = item.value()
        val jsonObject = ParseJsonData.parseObject(json)
  ...
  def calcVideoTime(key: String, iters: Iterable[LearnModel], sqlProxy: SqlProxy, client: Connection) = {
    ...
    learnList.foreach(item => {
      if ("".equals(interval_history)) {
        ...
        interval_history = tuple._2.toString()
        effective_duration_sum += effective_duration.toLong
        complete_duration_sum += complete_duration
